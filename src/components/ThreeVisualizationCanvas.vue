...
function createAxisHelpers(parentGroup, rawBounds) {
  ...
  // 计算标签位置：如果有包围盒数据，放在包围盒外面；否则放在轴末端
  const labelSize = bounds
    ? Math.max(
  ...
  // X标签：沿X轴方向，放在包围盒X最大值外侧
  const xLabel = createAxisLabel('X', '#ff4444', labelSize)
  xLabel.position.set(
    bounds
      ? bounds.xmax + labelOffset
      : axisOrigin.x + axisLength + labelOffset,
    axisOrigin.y,
    axisOrigin.z,
  )
  parentGroup.add(xLabel)

  // Y标签：沿Y轴方向，放在包围盒Y最大值外侧
  const yLabel = createAxisLabel('Y', '#44ff44', labelSize)
  yLabel.position.set(
    axisOrigin.x,
    bounds
      ? bounds.ymax + labelOffset
      : axisOrigin.y + axisLength + labelOffset,
    axisOrigin.z,
  )
  parentGroup.add(yLabel)

  // Z标签：沿Z轴方向，放在包围盒Z最大值外侧
  const zLabel = createAxisLabel('Z', '#4444ff', labelSize)
  zLabel.position.set(
    axisOrigin.x,
    axisOrigin.y,
    bounds
      ? bounds.zmax + labelOffset
      : axisOrigin.z + axisLength + labelOffset,
  )
  parentGroup.add(zLabel)
}
