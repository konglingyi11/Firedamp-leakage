...
class PeerStream extends HTMLVideoElement {
	constructor() {
		...
		this.dc = { send() { }, close() { }, readyState: "closed" };
	...
	async connectedCallback() {
		...
		this.ws.onerror = (err) => {
			console.warn("[peer-stream] WebSocket error, 将等待 close 后自动重连", err);
		};

		this.ws.onopen = () => {
			this._reconnectAttempt = 0;
			console.info("✅ peer-stream WebSocket 已连接", this.ws);
		...
		this.ws.onclose = (e) => {
			console.warn("[peer-stream] WebSocket 关闭", e.code, e.reason || "");
			try {
				if (this.dc && typeof this.dc.close === "function") this.dc.close();
			} catch (_) { }
			try {
				if (this.pc && typeof this.pc.close === "function") this.pc.close();
			} catch (_) { }
			this.dc = { send() { }, close() { }, readyState: "closed" };
			this.pc = { close() { } };

			this.dispatchEvent(
				new CustomEvent("playerdisconnected", {
			...
			const base = 2500;
			const delay = Math.min(30000, Math.round(base * Math.pow(1.6, attempt)));
			console.info(`[peer-stream] ${delay}ms 后尝试第 ${attempt + 1} 次重连…`);
