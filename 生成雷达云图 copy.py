...
def generate_custom_radar(width=910, height=795, frames=60, output_dir="sim/xz", bounds=None):
    ...
    hard_block_gate = block_side.astype(np.float32) * np.exp(-((X - cx) / (obstacle_w * 0.62)) ** 6)
    ...
    aperture_x = np.exp(-0.5 * (np.maximum(np.abs(X - cx) - obstacle_w * 0.42, 0.0) / (obstacle_w * 0.22 + 1e-9)) ** 2)
