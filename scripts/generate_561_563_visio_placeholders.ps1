...
$OutRoot = Join-Path $Root "output\doc\软件填写内容1_v2_5.6图稿"
...
function Add-Text($page, [double]$x, [double]$y, [double]$w, [double]$h, [string]$text, [int]$size = 18, [string]$color = "RGB(15,23,42)", [int]$bold = 1) {
  ...
  Set-CellFormula $s "Char.Font" 'FONT("Microsoft YaHei")'
...
function Setup-Page($doc, [string]$name, [string]$title, [string]$subtitle) {
  ...
  Add-Text $page 0.8 8.04 14.4 0.44 $title 22 | Out-Null
  Add-Text $page 0.8 7.72 14.4 0.25 $subtitle 10 "RGB(100,116,139)" 0 | Out-Null
  $sep = $page.DrawLine(0.8, 7.48, 15.2, 7.48)
  Set-CellFormula $sep "LineColor" "RGB(216,226,236)"
  Set-CellFormula $sep "EndArrow" "0"
  return $page
}
